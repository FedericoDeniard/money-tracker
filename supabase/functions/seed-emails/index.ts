...
Deno.serve(async req => {
  ...
  try {
    ...
    if (seedError || !newSeed) {
      ...
      return new Response(JSON.stringify({ error: "Failed to create seed" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
...
async function processChunk(
  ...
  if (seed.status === "completed" || seed.status === "failed") {
    return {
      ...
      processed: seed.last_processed_index,
  ...
  console.log(
    `Processing chunk: messages ${startIndex}-${endIndex - 1} of ${messageIds.length}`
  );
  ...
  try {
    ...
    if (aiResult.hasTransaction) {
      ...
      if (insertError) {
        ...
        console.log(
          `Transaction extracted by AI: ${transaction.amount} ${transaction.currency} - ${transaction.description}`
        );
...
async function updateSeedStatus(
  ...
  if (metadata) {
    if (metadata.totalMessages !== undefined)
      updateData.total_emails = metadata.totalMessages;
    if (metadata.processedMessages !== undefined)
      updateData.emails_processed_by_ai = metadata.processedMessages;
    if (metadata.transactionsFound !== undefined)
      updateData.transactions_found = metadata.transactionsFound;
    if (metadata.skippedMessages !== undefined)
      updateData.total_skipped = metadata.skippedMessages;
  }
