-- Improve cadence scoring by accepting monthly cadence with skipped billing cycles.
...
grouped AS (
  SELECT
    ...
    count(*) FILTER (
      WHERE i.interval_days IS NOT NULL AND (
        least(
          abs(i.interval_days - 30),
          abs(i.interval_days - 60),
          abs(i.interval_days - 90),
          abs(i.interval_days - 120)
        ) <= 7
      )
    )::integer AS monthly_interval_hits,
    count(*) FILTER (
      WHERE i.interval_days IS NOT NULL AND (
        least(
          abs(i.interval_days - 365),
          abs(i.interval_days - 730)
        ) <= 20
      )
    )::integer AS yearly_interval_hits,
...
component_scores AS (
  SELECT
    g.*,
    CASE
      WHEN g.monthly_interval_hits >= g.yearly_interval_hits AND g.monthly_interval_hits >= 1 THEN 'monthly'
      WHEN g.yearly_interval_hits > g.monthly_interval_hits AND g.yearly_interval_hits >= 1 THEN 'yearly'
      ELSE 'unknown'
    END AS frequency,
    CASE
      WHEN g.monthly_interval_hits >= g.yearly_interval_hits AND g.monthly_interval_hits >= 1 THEN (g.last_date + interval '30 days')::date
      WHEN g.yearly_interval_hits > g.monthly_interval_hits AND g.yearly_interval_hits >= 1 THEN (g.last_date + interval '365 days')::date
      ELSE NULL
