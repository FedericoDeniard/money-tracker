...
test(
  ...
  async () => {
    ...
    try {
      ...
      await stagehand.act(
        `type '${fakeUser.email}' into the email input field`
      );
      await stagehand.act(
        `type '${fakeUser.password}' into the password input field`
      );
      ...
      const errorData = await stagehand.extract(
        ...
        z.object({
          hasError: z
            .boolean()
            .describe("Whether there is an error message visible"),
          errorMessage: z
            .string()
            .optional()
            .describe("The error message text if visible"),
        })
      );
