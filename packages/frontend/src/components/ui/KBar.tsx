...
function KBarInner() {
  ...
  const authActions: Action[] = [
    ...
    },
    {
      ...
      shortcut: ["m"],
      keywords: t("kbar.keywords.metrics"),
      section: t("kbar.navigation"),
  ...
  return (
    <KBarPortal>
      <KBarPositioner className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/50 backdrop-blur-sm">
        <KBarAnimator className="w-full max-w-lg mx-4 overflow-hidden bg-white dark:bg-zinc-800 rounded-lg shadow-2xl border border-zinc-200 dark:border-zinc-700">
          <KBarSearch
            className="w-full h-12 px-4 text-sm bg-transparent border-0 border-b border-zinc-200 dark:border-zinc-700 outline-none placeholder:text-zinc-600 dark:placeholder:text-zinc-400 text-zinc-900 dark:text-zinc-100"
                  ...
                  {item.shortcut && (
                    <div className="flex gap-1">
                      {item.shortcut.map(key => (
                        <kbd
                          key={key}
                          className="px-2 py-1 text-xs bg-zinc-100 dark:bg-zinc-700 rounded border border-zinc-300 dark:border-zinc-600"
                        >
                          {key}
                        </kbd>
                      ))}
