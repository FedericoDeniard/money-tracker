...
const CustomizedContent = (props: CustomizedContentProps) => {
    const { x = 0, y = 0, width = 0, height = 0, index = 0, name = "", depth } = props;
    ...
    const percentage = props.percentage !== undefined
        ? Number(props.percentage)
        : (props.root?.value ? (value / props.root.value) * 100 : 0);
                ...
                <ResponsiveContainer width="100%" height={isMobile ? 300 : 320}>
                    ...
                    >
                        <Tooltip
                            ...
                            formatter={(value: number | string | readonly (string | number)[] | undefined, _: string | number | undefined, props: { payload?: ChartDataItem }) => {
                                const itemData = props.payload;
                                // In some complex shapes Recharts might wrap values in arrays, cast defensively
                                const numericValue = Array.isArray(value) ? value[0] : value;
                                return [`$${Number(numericValue || 0).toFixed(2)} (${itemData?.percentage?.toFixed(1) || 0}%)`, itemData?.name || ''];
                            }}
                        />
                    </Treemap>
                </ResponsiveContainer>
