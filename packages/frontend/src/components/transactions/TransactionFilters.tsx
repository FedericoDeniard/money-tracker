...
function DynamicFilterDropdowns({ filters, onFiltersChange }: DynamicDropdownsProps) {
  const { t } = useTranslation();
  const { currencies: availableCurrencies, emails: availableEmails } = useTransactionFilters();

  const updateFilter = (key: keyof TransactionFilters, value: string) => {
    const normalizedValue = value === "all" || value.trim() === "" ? undefined : value;
  ...
  const FilterDropdown = ({
    ...
    <div className="relative group">
      <div
        className={`flex items-center gap-2 px-3 py-1.5 md:py-2 bg-white border rounded-lg transition-all w-full sm:w-auto justify-between sm:justify-start h-8 md:h-9 ${value && value !== "all"
      ...
      >
        ...
        <ChevronDown size={14} className="opacity-50 transition-transform group-hover:translate-y-0.5 ml-auto" />
      </div>
      <select
        value={value || "all"}
...
export function TransactionFiltersComponent({
  ...
  const FilterDropdown = ({
    ...
    <div className="relative group">
      ...
      >
        <Icon size={14} className={`shrink-0 ${value && value !== "all" ? "text-[var(--primary)]" : "text-gray-500"}`} />
        <span className="text-xs md:text-sm font-medium whitespace-nowrap truncate">
          {value && value !== "all"
            ? options.find((o) => o.value === value)?.label || value
            : label}
  ...
  return (
    <section
      className={`rounded-2xl border border-[var(--text-secondary)]/20 bg-[var(--bg-primary)] p-4 md:p-6 shadow-sm transition-opacity duration-200 ${isLoading ? "opacity-60 pointer-events-none" : "opacity-100"
        }`}
    >
      <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
        ...
        <div className="shrink-0 mb-2 xl:mb-0">
          ...
          <p className="mt-1 text-xs md:text-sm text-[var(--text-secondary)] line-clamp-2 md:line-clamp-none">
            {t("transactions.pageDescription", "View and manage all your income and expenses in one place.")}
          </p>
        </div>
        ...
        <div className="flex flex-col xl:flex-row gap-3 xl:items-center xl:justify-end xl:flex-1 shrink-0">
          <div className="flex flex-row flex-wrap gap-2 items-center justify-start xl:justify-end w-full xl:w-auto">
            ...
            >
              ...
              <ChevronDown size={14} className={`opacity-50 transition-transform ${showDatePicker ? "rotate-180" : ""}`} />
            </Button>

            {/* Type Filter — static */}
            <div className="bg-[var(--bg-secondary)] p-1 rounded-lg flex items-center gap-1 w-full sm:w-auto overflow-x-auto shrink-0">
              {[
                { id: "all", label: t("transactions.allTypes"), icon: LayoutList },
                { id: "income", label: t("transactions.income"), icon: ArrowDownCircle },
                { id: "expense", label: t("transactions.expense"), icon: ArrowUpCircle },
              ].map((type) => {
            ...
            <Suspense fallback={<DynamicDropdownsSkeleton />}>
              <DynamicFilterDropdowns filters={filters} onFiltersChange={onFiltersChange} />
            </Suspense>
          ...
          {showDatePicker && (
            ...
            >
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-gray-500">{t("transactions.startDate")}</label>
                <input
                  type="date"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-gray-500">{t("transactions.endDate")}</label>
                <input
                  type="date"
